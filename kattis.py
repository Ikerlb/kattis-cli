...
_DEFAULT_CONFIG = os.path.join(os.path.expanduser('~'), ".config/.kattisrc")
_LANGUAGE_GUESS = {
    '.c': 'C',
    '.c++': 'C++',
    '.cc': 'C++',
...
def get_config():
    ...
    if not cfg.read([_DEFAULT_CONFIG,
...
def main():
    parser = argparse.ArgumentParser(prog='kattis', description='Submit a solution to Kattis, or fetch problem description and sample files')

    subparsers = parser.add_subparsers(dest = 'command')
    submit = subparsers.add_parser('submit', help = 'submit solution')

    submit.add_argument('-p', '--problem',
                        help=''''Which problem to submit to.
Overrides default guess (first part of first filename)''')
    submit.add_argument('-m', '--mainclass',
                        help='''Sets mainclass.
Overrides default guess (first part of first filename)''')
    submit.add_argument('-l', '--language',
                        help='''Sets language.
Overrides default guess (based on suffix of first filename)''')
    submit.add_argument('-t', '--tag',
                        help=argparse.SUPPRESS)
    submit.add_argument('-f', '--force',
                        help='Force, no confirmation prompt before submission',
                        action='store_true')
    submit.add_argument('files', nargs='+')

    fetch = subparsers.add_parser('fetch', help = 'fetch a problem')
    
    fetch.add_argument('-p', '--problem', help = '''Which problem to fetch.''')
...
def fetch_command(args):
    if args.problem is None: 
        print(f"Must pass problem to fetch.")
        sys.exit(1)
    
    problem = args.problem
    if Path(problem).exists():
        print(f"File or folder {problem} already exists.")
        sys.exit(1)
